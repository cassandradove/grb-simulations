...
class FixedLengthLIFOQueue:
    def __init__(self, length):
        ...
        self.variance = 0                       # Initialize variance

    def push(self, item):
        if len(self.queue) == self.queue.maxlen:
            self.variance -= ((self.queue[-1] - self.get_running_average()) ** 2) / len(self.queue)
        ...
        self.variance += ((item - self.get_running_average()) ** 2) / len(self.queue)
